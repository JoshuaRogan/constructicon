...
describe('LazyImage', () => {
  it('intially rendered without a background image and with correct state', () => {
    ...
    expect(wrapper.instance().state.loaded).to.eql(false)
  ...
  it('correctly sets the background image once loaded', () => {
    ...
    return Promise.resolve()
      ...
      .then(() => {
        ...
        expect(wrapper.dive().instance().state.loaded).to.eql(true)
  ...
  it('delays loading the backgroundImage if lazy is true', () => {
    ...
    return Promise.resolve()
      ...
      .then(() => {
        const style = wrapper.dive().find('.c11n-lazy-image').get(0).props.style
        expect(wrapper.dive().instance().state.loaded).to.eql(false)
        expect(style).to.have.property('backgroundImage', '')
      })
  })
})
